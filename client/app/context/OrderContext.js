...
export const OrderProvider = ({children}) => {
    ...
    const fetchActiveOrders = useCallback(async (userId) => {
        try {
            const response = await axios.get(`/orders/active/${userId}`);
    ...
    const moveToHistory = useCallback((order) => {
        setActiveOrders(prev => {
            const newActive = {...prev};
            delete newActive[order._id];
            return newActive;
        });
        setHistoricalOrders(prev => ({...prev, [order._id]: order}));
    }, []);
